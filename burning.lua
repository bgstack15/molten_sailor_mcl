...
minetest.register_globalstep(function(dtime)
	...
	if timer >= 1 then
		...
		for _,player in ipairs(minetest.get_connected_players()) do
			...
			local has_pants = inv:contains_item("armor","molten_sailor_mcl:leggings_obsidian")
			local has_boots = inv:contains_item("armor","molten_sailor_mcl:boots_obsidian")
			local has_full_suit = molten_sailor_mcl.has_full_lava_suit(player)

			-- does the player wear a suit?
			molten_sailor_mcl.set_player_wearing(player, has_full_suit, has_helmet, armor_inv)

			-- is player on fire?
			local pos = player:get_pos()
			local on_fire = minetest.find_node_near(pos, 1, {"group:lava","group:fire"})

			if on_fire and (has_full_suit or has_helmet or has_chestplate or has_pants or has_boots) then
